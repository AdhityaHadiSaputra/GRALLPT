...
class _ScanQRDefectPageState extends State<ScanQRDefectPage> {
  ...
  late TextEditingController qtyControllerpo;
  late TextEditingController qtyControllermaster;
  late TextEditingController qtyControllernoitem;
  ...
  for (var i = 0; i < scannedResults.length; i++) {
     ...
     for (var i = 0; i < masterScannedResults.length; i++) {
      _controllers1[i] = TextEditingController(
        text: masterScannedResults[i]['qty_scanned'].toString(),
      );
     }
...
Future<void> updatePO(Map<String, dynamic> item) async {
  ...
  ...scannedResults.asMap().entries.map(
    ...
    //     DataCell(
    //       TextFormField(
    //         controller: qtyControllerpo, // Use controller here
    //         keyboardType: TextInputType.number,
    ...
    //             Flushbar(
    //     message: 'Tidak Boleh Kurang Dari 1',
    //     duration: Duration(seconds: 3),
    //     flushbarPosition: FlushbarPosition.TOP,
    //     backgroundColor: Colors.red,
    // ).show(context);

    //             // Reset the text field to the last valid value
    //             qtyControllerpo.text = mutableResult['qty_scanned'].toString();
    //           }
    //         },
    //       ),
    //     ),
 ...
 ...masterScannedResults.asMap().entries.map(
          (entry) {
       int index = entry.key;
            Map<String, dynamic> result = entry.value;
            Map<String, dynamic> mutableResult = Map.from(result);
